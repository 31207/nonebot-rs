    ...
    async fn handle(&self, event: NoticeEvent, matcher: Matcher<NoticeEvent>) {
        match event {
            NoticeEvent::Notify(m) => {
                matcher.send(UniMessage::new().text(format!("收到通知事件: {}", m.user_id).as_str()).build())
                .await;
            },
            NoticeEvent::GroupRecall(m) => {
                matcher.send(UniMessage::new().text(format!("收到群消息撤回事件: {}", m.user_id).as_str()).build())
                .await;
            },
            NoticeEvent::FriendRecall(m) => {
                matcher.send(UniMessage::new().text(format!("收到好友消息撤回事件: {}", m.user_id).as_str()).build())
                .await;
            },
            NoticeEvent::GroupIncrease(m) => {
                matcher.send(UniMessage::new().text(format!("收到群成员增加事件: {}", m.user_id).as_str()).build())
                .await;
            },
            NoticeEvent::GroupDecrease(m) => {
                matcher.send(UniMessage::new().text(format!("收到群成员减少事件: {}", m.user_id).as_str()).build())
                .await;
            },
            NoticeEvent::GroupBan(m) => {
                matcher.send(UniMessage::new().text(format!("收到群成员禁言事件: {}", m.user_id).as_str()).build())
                .await; 
            }
        }
    }
